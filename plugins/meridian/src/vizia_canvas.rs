...
thread_local! {
    ...
    static ENVELOPE_POINTS: RefCell<Vec<(f32, f32)>> = RefCell::new(Vec::new());
}
            ...
            |c| {
                ENVELOPE_POINTS.with(|buf| {
                    ...
                    if points.len() >= 2 {
                        ...
                        fb.close();
                        c.draw_path(&fb.detach(), &fill_paint(col(1.0, 0.35, 0.15, 0.18)));
                        ...
                        c.draw_path(&lb.detach(), &stroke_paint(rgb(1.0, 0.4, 0.2), 1.2));
                    }
                });
                ...
                if self.peak_hold > 0.1 {
                    ...
                    while x < w - margin {
                        ...
                        line(c, x, py, ex, py, col(1.0, 0.65, 0.15, 0.55), 1.0);
                        x += 7.0;
                    }
                }
            },
