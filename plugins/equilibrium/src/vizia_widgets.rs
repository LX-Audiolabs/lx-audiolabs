//! Vizia/Skia port of `shared-ui/src/widgets.rs`'s knob/slider `canvas::Program`s,
//! scoped to what Equilibrium's editor uses. Same duplication rationale as
//! `plugins/lucent/src/vizia_widgets.rs`: lives here until the Vizia pilot
//! moves to a shared crate (CLAP-vault features/2026-07-04-truce-2.0-upgrade-plan.md
//! says not before the second port, Aurum).
//!
//! `KnobView`/`Gesture`/`format_knob_value` are a byte-identical copy of
//! Lucent's — the widget is generic over min/max/bipolar and needed no
//! Equilibrium-specific change. `HSliderView` is new: a straight port of
//! `shared-ui::HSliderProgram` (drag-to-position, not delta-based like the
//! knob, since a horizontal track's value maps directly from cursor x).
...
/// Formats a knob's real-unit value for the readout label under it - same
/// three-tier formatting `knob_gesture` used (k-suffix above 1000, no
/// decimals above 100, 2 decimals below).
...
/// Horizontal drag slider - port of `shared-ui::HSliderProgram`. Unlike
/// `KnobView` (delta-based drag), the value maps directly from the cursor's
/// x position within the track, matching the iced original's `val_at`.
...
impl HSliderView {
    pub fn new(
        ...
        // Bipolar: only when the range crosses zero (Gain -12..+12, Pan -1..+1).
...
impl View for HSliderView {
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &vg::Canvas) {
        ...
        if self.hovered {
            ...
            ring.set_color(rgba(1.0, 0.45, 0.1, 0.6));
