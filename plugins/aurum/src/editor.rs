...
fn tog<'a>(cx: &'a mut Context, lbl: &'static str, act: bool, on: impl Fn(&mut EventContext) + 'static + Send + Sync) -> Handle<'a, impl View> { let s = Signal::new(act); Button::new(cx, move |cx| Label::new(cx, lbl).font_size(9.0)).on_press(move |cx| { s.update(|v| *v = !*v); on(cx); }).width(Pixels(52.0)).height(Pixels(22.0)).class("lx-btn").toggle_class("active", s) }
...
pub fn build(cx: &mut Context, lens: ParamLens<AurumParams>, params: Arc<AurumParams>, shared: Arc<SharedState>) {
    ...
    VStack::new(cx, move |cx| {
        ...
        HStack::new(cx, move |cx| {
            ...
            for (i, lb) in ["SHAPE", "COLOR", "LIMIT"].iter().enumerate() { let ts = tab; let idx = i; Button::new(cx, move |cx| Label::new(cx, *lb).font_size(12.0)).on_press(move |_cx| ts.set(idx)).width(Pixels(70.0)).height(Pixels(28.0)).class("lx-btn").toggle_class("active", Memo::new(move |_| tab.get() == idx)); }
            Element::new(cx).width(Stretch(1.0));
        ...
        HStack::new(cx, move |cx| {
            ...
            VStack::new(cx, move |cx| {
                ...
                let sr2 = sr.clone(); Button::new(cx, |cx| Label::new(cx, "RESET PEAK").font_size(9.0)).on_press(move |_cx| sr2.reset_peak.store(true, Ordering::Relaxed)).width(Stretch(1.0)).height(Pixels(24.0)).class("lx-btn");
        ...
        HStack::new(cx, move |cx| {
            ...
            VStack::new(cx, move |cx| {
                ...
                HStack::new(cx, move |cx| {
                    ...
                    Button::new(cx, |cx| Label::new(cx, "RESET").font_size(11.0)).on_press(move |_cx| { se.reset_peak.store(true, Ordering::Relaxed); set(&le, P::ClipCeiling, 0.8167); set(&le, P::OutputGain, 0.5); set(&le, P::StereoWidth, 0.5); }).width(Pixels(60.0)).height(Pixels(26.0)).class("lx-btn");
