// Equilibrium (egui prototype) — Pre-master spectral balancer (truce port).
//
// PROTOTYPE: same DSP as `plugins/equilibrium`, editor rewritten on
// truce-egui instead of truce-vizia/shared-ui, to evaluate framework
// compatibility. Not a feature-complete port - see `editor.rs` module doc
// for what's deliberately left out (presets/vault/snap/listen/goniometer).
//
