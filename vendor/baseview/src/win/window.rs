        ...
        B: Send + 'static,
    {
        unsafe {
            ...
            *window_state.handler.borrow_mut() = Some(Box::new(handler));
            ...
            // lx-audiolabs: only set this for our OWN top-level window
            // (the `!parented` / standalone-app case). A parented window
            // is embedded into a host process (a DAW) that already set
            // its own process-wide DPI awareness at its own startup -
            // `SetProcessDpiAwarenessContext` can only be set once per
            // process and is documented to silently fail if called
            // again, but unconditionally calling it here regardless of
            // `parented` still executes a process-wide Win32 API call
            // from inside the embedded plugin's GUI-open path, which is
            // never this plugin's state to own. Confirmed by testing:
            // this call site correlates with an editor hang when
            // embedded in Reaper on this AMD-iGPU test machine that
            // does not reproduce for a standalone (`!parented`) window.
            // Root-caused via source review (CLAP-vault
            // features/2026-07-04-truce-2.0-upgrade-plan.md), not yet
            // proven to be the sole cause - see that doc for the
            // parallel GL-context-on-AMD suspicion this doesn't rule out.
            if !parented {
