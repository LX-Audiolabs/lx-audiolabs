...
impl<'a> DpiAwarenessContext<'a> {
    pub fn new(user32: &'a ExtendedUser32) -> Result<Self> {
        Self::new_internal(user32, true)
    }

    /// Like [`Self::new`], but does not change the calling thread's DPI
    /// awareness context. Used for embedded (parented) plugin windows where
    /// the host process already owns the thread/process awareness and
    /// changing it from the plugin has been observed to hang REAPER on some
    /// systems.
    pub fn new_parented(user32: &'a ExtendedUser32) -> Result<Self> {
        Self::new_internal(user32, false)
    }

    fn new_internal(user32: &'a ExtendedUser32, set_awareness: bool) -> Result<Self> {
        if !set_awareness {
