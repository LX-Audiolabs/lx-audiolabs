    ...
    fn after_create(&self, window: HWnd) -> Result<()> {
        ...
        if dpi != window_state.current_dpi.get() {
            ...
            if let WindowScalePolicy::SystemScaleFactor = self.window_state.scale_policy {
                ...
                window.resize_and_activate(new_size, dpi, &window_state.user32, window_state.parented)?;
            }
        }
...
unsafe fn wnd_proc_inner(
    ...
    match msg {
        ...
        WM_DPICHANGED => {
            ...
            let dpi_ctx = if window_state.parented {
                DpiAwarenessContext::new_parented(&window_state.user32).unwrap()
            } else {
                DpiAwarenessContext::new(&window_state.user32).unwrap()
            };
...
impl Window {
    ...
    fn open<H: WindowHandler>(
        ...
        let dpi_ctx = if parented {
            DpiAwarenessContext::new_parented(&extended_user_32).unwrap()
        } else {
            DpiAwarenessContext::new(&extended_user_32).unwrap()
        };
        ...
        let initializer = {
            ...
            move |hwnd: HWnd| {
                let window_state =
                    Rc::new(WindowState::new(hwnd, window_size, options.scale, extended_user_32, parented));
