...
/// tries DX12 first, falling back to GL (native WGL) when the host has
/// no DX12 adapter at all - old GPUs/drivers, or a DX12 init failure.
/// `Backends` is a bitflag `request_adapter` picks an adapter from;
/// with zero DX12 adapters enumerated it silently returns the GL one
/// instead of leaving the editor blank/dead. DX12 stays preferred
/// (`HighPerformance` in the adapter request) - see
/// `bugs/all/2026-07-04-multi-instance-vram-exhaustion-freeze.md` in
/// the vault for why GL is fallback-only, not primary. macOS is Metal,
/// Linux keeps `PRIMARY`. Keep the two copies in sync.
#[must_use]
