...
/// stays preferred (`HighPerformance` in the adapter request) - see
/// `bugs/all/2026-07-04-multi-instance-vram-exhaustion-freeze.md` in
/// the vault for why GL is fallback-only, not primary (worse under
/// concurrent multi-instance load on at least one tested AMD iGPU).
/// Linux keeps `PRIMARY`.
#[must_use]
