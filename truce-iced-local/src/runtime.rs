...
impl<P: Params + 'static, M: IcedPlugin<P>> IcedRuntime<P, M> {
    ...
    fn finish_init(
        ...
        // MSAA render targets cost VRAM per sample regardless of backend
        // (MSAAx4 = 4x the color+resolve target memory). Each editor
        // instance owns an independent wgpu device (no sharing across
        // plugin instances), so N plugins open on one track multiply this
        // by N - on GPUs with small dedicated VRAM (e.g. 2GB iGPUs), 3
        // plugins open at once exhausted it and froze the host. Disabled
        // instead of downgraded to MSAAx2 to remove the multiplier
        // entirely; canvas edges/widget borders lose some smoothing, text
        // rendering is unaffected (glyph rasterization is a separate path).
